...
Para ejecutar la api en ambiente productivo o desarrollo debemos crear los archivos ```.env.production``` y ```.env.development``` para eso podemos seguir los comandos o descargar los secretos usando el siguente enlace: 
* production: [doppler.production](https://share.doppler.com/s/y6ewlmxdi4io4kdfp9agzb66olfkkowv25cy57cx#mahv23eQjZBGFAufX7cDvpfcO7pJSXUcypi6zSOiwiqCLGX2DvPtjBUUSsMgjN4B) 

* development: [doppler.development](https://share.doppler.com/s/atpwc7qhskwmwle6bhwpkntmrxnssvxhf28ptr4g#N3PhkCtXBbrHK3pdMNsGzUku6v0hIVRHdaFULZxEck4EYqmTOdFGnVErFC34TWGg) 


Si lo quieres hacer con comando, vamos a copiar los secretos del archivo ```.env.example``` en ```.env.production``` y ```.env.development```.

Recomendación: debemos asegurarnos que la variable ```MONGODB_URL``` en production apunte al contenedor de docker para este caso se llama: mongodb
```bash
#copiar las env del archivo .env.example a .env.production
$ Step 1. cp .env.example .env.production

#copiar las env del archivo .env.example a .env.development
$ Step 2. cp .env.example .env.development
```

## Run the application

nota: tener instalado docker y node
...
Utilice el framework [Jest](https://jestjs.io/docs/getting-started), por su facilidad de usarlo y por su capacidad de generar un ambiente sandbox, para cada prueba, asegura el aislamiento de los servicios teniendo que importar las env para su funcionamiento.

Estos unit tests se ejecutan en el ambiente de desarrollo ```NODE_ENV=development```
...
Descargar la collection de [Postman](https://drive.google.com/file/d/17ctHkXrgmsVt_ocYSCGkfvXz8RYWFC5r/view?usp=sharing) o guíese con la siguente información:

```bash
# create product

http: post
url: localhost:${PORT}/api/product
body:
{
  name: string,
  type: string,
...
# get one product
http: get
url: localhost:${PORT}/api/product/{id}
pathParameters:
	- id = string
...
cuando se elimina el producto retorna toda la información más la fecha de eliminación.
```

## Convención

Use <b>Kebab Case</b>, por su legibilidad para visulizar y entender los arhivos separando las palabras por guiones (-).

Use <b>Pascal Case</b>, para la clase coneción a la base de datos de mongodb.

Use <b>Camel Case</b>, para nombrar funciones o variables.

Use <b>Upper Case</b>, para nombrar las constantes que estan ubiadas en /src/@common/constanst 
